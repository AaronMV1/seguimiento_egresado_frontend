...
// Función para limpiar nombres de columna (remover saltos de línea)
function cleanColName(name) {
  return name.replace(/\n/g, '').replace(/\r/g, '').trim();
}

// Limpiar las claves del objeto
const cleanedData = data.map(row => {
  const cleanedRow = {};
  Object.keys(row).forEach(key => {
    cleanedRow[cleanColName(key)] = row[key];
  });
  return cleanedRow;
});
...
const sedesMap = {
  ...
  'Filial Ica': 2,
  'Filial Chincha': 3,
  'Otro (Lima norte. etc)': 1
};

// Mapeo de carreras a [facultad_id, carrera_id]
const carrerasMap = {
  'Medicina Humana': [1, 1],
  'Enfermería': [1, 2],
  'Estomatología': [1, 3],
  ...
  'Ingeniería de Computación y Sistemas': [2, 8],
  ...
  'Facultad de Ingenierías': [2, 8],
  'Facultad de Ciencias de la Salud': [1, 1],
  'Facultad de Derecho y Ciencias Empresariales': [3, 12],
  'Facultad de Comunicación y Ciencias Administrativas': [4, 18]
};
...
function mapGenero(generoForm) {
  ...
  const lower = String(generoForm).toLowerCase();
...
function getCarreraInfo(carreraStr) {
  if (!carreraStr) return [1, 1];
  const key = String(carreraStr).trim();
  if (carrerasMap[key]) {
    return carrerasMap[key];
  }
  // Buscar coincidencia parcial
  for (let mapKey in carrerasMap) {
    if (key.includes(mapKey) || mapKey.includes(key)) {
      return carrerasMap[mapKey];
    }
  }
  return [1, 1]; // Default
}
...
cleanedData.forEach((row, index) => {
  ...
  try {
    ...
    const nombresApellidos = row['Ingrese sus nombres y apellidos'] || 'Desconocido';
    const genero = mapGenero(row['Género']);

    const sedeKey = row['¿En que sede /filial estudió?'] || 'Local Chorrillos';
    const sedeId = sedesMap[sedeKey] || 1;

    const carreraKey = row['Indique a que Escuela Profesional pertenece'] || '';
    const [facultadId, carreraId] = getCarreraInfo(carreraKey);
    ...
    if (fase === 2) {
      ...
      const fase2_empresaempleadornumero = '';
    ...
    if (fase === 4) {
      ...
      const fase4_participacion = '';
      const fase4_resultados = '';
